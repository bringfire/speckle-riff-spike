...
`rook_canonical_ghx_sha256_v1` is SHA-256 over a Rook-owned canonical in-memory GHX serialization of the complete active document using the pinned Grasshopper runtime. The same capture implementation and runtime produce the reviewed token and recompute it at action time. Cosmetic state may conservatively invalidate it; selection and viewport state absent from GHX are not execution state. Gate 1 must demonstrate that object addition/removal, wiring, persistent values, script source, and component/pin configuration change the token. Unknown algorithms and runtime mismatches refuse preflight.
...
Refuse zero or multiple candidates, pending/corrected/rejected nodes, altered sidecar bytes, source-version mismatch, target or cross-field mismatch, missing/duplicate/malformed reviewed binding, unknown fingerprint algorithm, fingerprint/runtime mismatch, unit mismatch, comparison mismatch, unauthorized proposed value, or consumed authorization before permit creation. These are preflight refusals and create no `AgenticChangeRecord`. After permit creation and executor invocation, a live document ID, whole-document fingerprint, slider GUID, control type, parameter, ambiguity, or `before` mismatch performs no mutation but produces `AgenticChangeRecord.outcome: failed` and cannot be retried.
...
Validate each fixture against its contract. Demonstrate preflight acceptance only for one accepted candidate and demonstrate refusal for every listed pre-permit case. Compare the executor fixture field-for-field with the authorizing payload; prove the fingerprint changes for the required document mutations; test successful compare-and-set plus live document-state, missing, stale, wrong-type, and ambiguous resolution as post-invocation failed outcomes with no mutation; test concurrent same-key and different-hash/same-candidate permit creation; and calculate deterministic comparisons from decimal textual values without conversion.
